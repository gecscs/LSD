...
namespace LayeredSelectionDisplay.Extensions
{
    ...
    public abstract partial class ExtendedUISystemBase : UISystemBase
    {
        ...
        {
            var helper = new ValueBindingHelper<T>(new (LayeredSelectionDisplayMod.Id, key, initialValue, new GenericUIWriter<T>()));
        ...
        {
            var helper = new ValueBindingHelper<T>(new (LayeredSelectionDisplayMod.Id, key, initialValue, new GenericUIWriter<T>()), updateCallBack);
            var trigger = new TriggerBinding<T>(LayeredSelectionDisplayMod.Id, setterKey, helper.UpdateCallback, initialValue is Enum ? new EnumReader<T>() : null);
        ...
        {
            var binding = new GetterValueBinding<T>(LayeredSelectionDisplayMod.Id, key, getterFunc, new GenericUIWriter<T>());
        ...
        {
            var binding = new TriggerBinding(LayeredSelectionDisplayMod.Id, key, action);
        ...
        {
            var binding = new TriggerBinding<T1>(LayeredSelectionDisplayMod.Id, key, action);
    ...
    /// <returns>A trigger binding for the specified key and action with two parameters.</returns>
        ...
        {
            var binding = new TriggerBinding<T1, T2>(LayeredSelectionDisplayMod.Id, key, action);
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3>(LayeredSelectionDisplayMod.Id, key, action);
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3, T4>(LayeredSelectionDisplayMod.Id, key, action);
